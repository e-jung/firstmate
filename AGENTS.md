...
config/daemon.conf   fm-supervise-daemon tunables, sourced at startup; LOCAL, gitignored; managed by bin/fm-config.sh (see PLUGINS.md)
...
state/               volatile runtime signals; gitignored
  ...
  <id>.check.sh      optional slow poll you write per task (e.g. merged-PR check); for fleet-wide event sources (disk health, GitHub PRs, a Linear inbox) use bin/fm-plugin.sh, which generates a contract-correct wrapper (see PLUGINS.md)
...
- **Portable singleton lock** — the daemon uses the repo's mkdir-based lock helper (`fm-wake-lib.sh`) instead of `flock`, which is absent on macOS. The lock and state root are anchored to the **canonical firstmate home** (selected by `FM_HOME`, then `FM_STATE_OVERRIDE`, then `FM_ROOT_OVERRIDE`), and the daemon **refuses to start when launched from a git worktree with no explicit canonical selector**: the script-dir fallback would otherwise lock a worktree-local `state/.supervise-daemon.lock` (a different inode than the canonical daemon's) and bypass the singleton, which is the 2026-06-22 double-daemon freeze root cause. An explicit `FM_HOME` / `FM_STATE_OVERRIDE` / `FM_ROOT_OVERRIDE` is trusted even from a worktree.
