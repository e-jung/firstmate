...
The supervisor target is `FM_SUPERVISOR_TARGET` (default `firstmate:0`); the daemon is single-instance via `flock`, traps SIGTERM/SIGINT for a clean shutdown (within ~1s, releasing the lock so an immediate restart succeeds), logs each wake to `state/.supervise-daemon.log` (size-capped), and restarts a crashing watcher rather than dying (a tight crash-restart spin is detected and backed off).
`FM_INJECT_SKIP` (default `heartbeat`) is a `|`-separated list of reason-prefixes the daemon logs but does NOT inject, so routine heartbeats don't churn a no-op supervisor turn; actionable `signal`/`stale`/`check` wakes still inject. If the supervisor pane disappears mid-run the daemon backs off and retries rather than crashing or swallowing wakes (it does not run the watcher while the pane is gone, since the watcher marks a signal seen before exiting).
