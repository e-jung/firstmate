...
4. `check:` a check-script fired - a per-task poll (usually a merge, or X mode when enabled) or a fleet-wide GitHub PR event from `bin/fm-github-watch.sh` (new comments, reviews, CI flips, or a merge/close on any of the contributor's open PRs; `docs/scripts.md`); act on it.
