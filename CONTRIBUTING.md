...
tests/fm-fleet-view.test.sh               # fm-fleet-view.sh compact in-flight task table (stable header, crew-state status summary, PR/report pointers, paths, watch/steer/codex-link actions, backlog-only recovery rows, queued-row exclusion, empty fleet) and fm-codex-link.sh codex://new deep-link generation for task worktrees, scout reports, and project dirs with URL encoding and missing-task rejection
