...
FM_GH_CONTRIBUTOR=           # GitHub login whose open PRs fm-github-watch.sh polls; unset means the authenticated gh user
FM_GH_POLL_SECS=300          # seconds between polls in fm-github-watch.sh --daemon mode
FM_GH_CONCURRENCY=8          # max PRs fm-github-watch.sh polls concurrently per sweep; 0 or non-numeric falls back to 8
FM_GH_CLOSE_REPROBE_SECS=7200   # seconds after a PR closes that fm-github-watch.sh keeps re-probing for a reopen->merge
FM_GH_IGNORE_CHECKS=PR must be raised via no-mistakes   # regex of check-run names dropped from fm-github-watch.sh CI roll-up; empty disables
