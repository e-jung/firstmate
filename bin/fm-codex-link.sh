...
#
# The Codex app supports codex://new?path=<absolute-dir>&prompt=<text>. This
# helper never opens links or mutates state; it resolves known task/report paths
# and prints both the deep link and the raw local path so the captain has a
# fallback if the app is not registered on the current device.
#
# Usage:
