...
#          recovered and STUCK clone drift, prunes gone local branches, and
#          fast-forwards no-mistakes gate forks from their upstream parent
#          (sync_fork_default in fm-fleet-sync.sh); it is bounded by FM_FLEET_SYNC_BOOTSTRAP_TIMEOUT when it is a non-empty
...
#          github_watch_setup, fleet_sync) while still printing every read-only
#          detect line
...
# GitHub PR review/CI watcher: arm the check shim that surfaces new comments,
# changes-requested reviews, and failed CI on the PRs Firstmate is supervising.
# Same registration path as x_mode_setup - an idempotent state/*.check.sh the
# existing watcher sweeps - but not opt-in: it is core supervision, and it
# self-no-ops (zero gh calls) when no state/*.meta carries a pr= line, so a home
# with no PRs in flight pays nothing. The shim execs bin/fm-github-watch.sh,
# which discovers supervised PRs from authoritative state and degrades safely
# on any gh/auth/network failure. Silent on success (silent = all good).
github_watch_setup() {
  local shim shim_body
  [ -f "$SCRIPT_DIR/fm-github-watch.sh" ] || return 0
  shim="$STATE/github-events.check.sh"
  shim_body=$(cat <<EOF
#!/usr/bin/env bash
# Auto-generated by fm-bootstrap.sh - GitHub PR events check shim.
# The watcher runs this each check cycle; stdout becomes a check: wake that
# escalates new review comments, changes-requested reviews, and failed CI on
# the PRs Firstmate is supervising. Silent when nothing changed.
export FM_HOME=$(printf '%q' "$FM_HOME")
exec $(printf '%q' "$FM_ROOT/bin/fm-github-watch.sh") --once
EOF
)
  write_if_changed "$shim" "$shim_body" 2>/dev/null || return 0
