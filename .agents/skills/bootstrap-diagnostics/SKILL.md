...
- `FLEET_SYNC: <repo>: fork-sync: skipped: diverged from <parent>` - a no-mistakes gate fork has unique commits that diverge from its upstream parent, so the automatic fast-forward sync was skipped to avoid losing fork-unique work; the gate base will stay stale until the divergence is resolved manually.
  Non-fatal: the gate still runs, just against a behind base; investigate when convenient.
- `FLEET_SYNC: <repo>: fork-sync: skipped: <error>` - the fork sync failed (API error, offline, rate limit, merge-upstream non-fast-forward); non-fatal and retried next session.
