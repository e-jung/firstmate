...
EOF
  ...
  # Remove a firstmate-specific axi tool (lives in ~/.local/bin, never in the
  # system dirs BASE_PATH covers) so the MISSING is deterministic regardless of
  # what the host has installed under /usr/bin (e.g. node/gh/tmux leak there).
...
EOF
  make_fake_toolchain "$fakebin"
  make_fake_ps_claude "$fakebin"
  # Remove a firstmate-specific axi tool (not in BASE_PATH's system dirs) so the
  # MISSING line is deterministic regardless of host-installed node/gh/tmux.
  rm -f "$fakebin/lavish-axi"
