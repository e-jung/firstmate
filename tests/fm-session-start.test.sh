...
EOF
  ...
  # lavish-axi is a fake-provided tool guaranteed absent from BASE_PATH
  # (/usr/bin:/bin:/usr/sbin:/sbin), unlike node which may live in /usr/bin.
...
EOF
  make_fake_toolchain "$fakebin"
  make_fake_ps_claude "$fakebin"
  # lavish-axi is fake-provided and guaranteed absent from BASE_PATH, so
  # removing it deterministically forces a MISSING line (node would resolve
  # from /usr/bin on hosts that have it installed).
  rm -f "$fakebin/lavish-axi"
