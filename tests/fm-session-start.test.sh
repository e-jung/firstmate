...
EOF
  make_fake_toolchain "$fakebin"
  make_fake_ps_claude "$fakebin"
  # The bootstrap section is non-trivial: tasks-axi is not faked and not on the
  # host BASE_PATH, so bootstrap deterministically reports MISSING: tasks-axi
  # (the same convention fm-bootstrap.test.sh relies on). node is NOT used here
  # because /usr/bin/node (Linux) would shadow a removed fake node.
...
EOF
  ...
  # fm-bootstrap.sh's own exact MISSING-tool line format. tasks-axi is used
  # (not node) because it is deterministically absent from BASE_PATH on every
  # host, whereas /usr/bin/node on Linux would shadow a removed fake node.
